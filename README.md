...
- **Ready to harvest:** a solid box in the crop's color with a white check mark, visible from the side and from above. It pulses between the crop color and white (about once per second).
...
Everything under `assets/` is generated by `tools/gen.py` (Python 3 + Pillow). Edit the `CROPS` table (`color`, `pulse`, `motif`), then run `python tools/gen.py` and `python tools/validate.py`.
